...
let passmysql = "tjPFV0q0bh22rnUHQd"
...
let port = 'port'
let value6 = await interaction.options.getString("port")
...
.then(async (res)  => {
  ...
  if (res.data.status == 200) {
    database.query(`UPDATE ogp_user_homes SET access_rights='tupecf' WHERE home_id = '${res.data.message}'`, async function(error, results) {
      if (error) throw error;
    });
  }
})
